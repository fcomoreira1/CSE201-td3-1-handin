...
void sort(double *&array, const int arr_len) {
    for (int i = 0; i < arr_len - 3; i += 3) {
        for (int j = 0; j < arr_len - i - 3; j += 3) {
            if (array[j] > array[j + 3]) {
                std::swap(array[j], array[j + 3]);
                std::swap(array[j + 1], array[j + 4]);
                std::swap(array[j + 2], array[j + 5]);
            }
        }
    }
}
...
void merge_telemetry(double **telemetries, int tot_telemetries,
                     int *telemetries_sizes, double *&global_telemetry,
                     int &global_telemetry_current_size,
                     int &global_telemetry_max_size) {
    for (int i = 0; i < tot_telemetries; i++) {
        for (int j = 0; j < telemetries_sizes[i]; j++) {
            global_telemetry = append_to_array(
                telemetries[i][j], global_telemetry,
